...
![thesis.drawio.png](/ichnaea/thesis.drawio.png)
...
I've set the sampling frequency to 10HZ, just enough to get frequent updates on the body position without bogging down the gateway. Things are pretty straightforward from this point on. The client establishes a socket connection with the backend and synchronously transfers data.
...
![Untitled](/ichnaea/influx-chart.png)
...
The main point of the dashboard is the model animation. I fixed a [Mixamo](http://mixamo.com) model to a Three.js scene, and with the help of [quaternions](https://en.wikipedia.org/wiki/Quaternion), the model animation is a breeze. I used the build in function from Three.js for the conversion from Euler, setFromEuler(). Taking a look inside setFromEuler(), we can see that the conversion is simple enough.
